    ...
    <div className="h-full flex flex-col animate-fadeIn">
      ...
      <div className="flex bg-white p-1 rounded-2xl mb-4 w-full overflow-x-auto no-scrollbar border border-slate-200 shadow-sm sticky top-0 z-30 shrink-0">
        {[
          ...
          { id: 'REQUESTS', label: 'Yêu cầu', icon: <ArrowRightLeft size={16}/>, count: qrRequests.length + moveRequests.length + paymentRequests.length + billingTables.length },
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24 px-1">
        ...
        {activeTab === 'REQUESTS' && (
           <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-indigo-100 shadow-sm">
                   ...
                   <div className="space-y-3">
                      {billingTables.map((t:any) => (
                         <div key={t.id} className="flex items-center justify-between p-4 bg-indigo-50 rounded-2xl border border-white">
                   ...
                   <div className="space-y-3">
                      {moveRequests.map((n:any) => (
                         <div key={n.id} className="flex flex-col sm:flex-row items-center justify-between p-4 bg-blue-50 rounded-2xl border border-white gap-3">
                            <div><p className="text-[11px] font-black uppercase italic leading-none">{n.message}</p></div>
                            <button onClick={() => store.approveTableMove(n.id)} className="w-full sm:w-auto bg-blue-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase italic shadow-lg">Duyệt</button>
                         </div>
                      ))}
                   </div>
                </div>
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                 <h4 className="font-black text-xs uppercase italic mb-6 flex items-center gap-2 text-amber-500"><CreditCard size={18}/> Yêu cầu thanh toán ({paymentRequests.length})</h4>
                 <div className="space-y-3">
                    {paymentRequests.map((t:any) => (
                       <div key={t.id} className="flex items-center justify-between p-4 bg-amber-50 rounded-2xl border border-amber-100">
                          <p className="text-[11px] font-black uppercase italic leading-none">Bàn {t.id === 0 ? 'Khách lẻ' : t.id}</p>
                          <button onClick={() => store.confirmPayment(t.id)} className="bg-amber-600 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase italic shadow-lg">Xác nhận</button>
                       </div>
                    ))}
                    {paymentRequests.length === 0 && <p className="text-center py-6 text-slate-300 text-[10px] font-black uppercase italic leading-none">Không có yêu cầu</p>}
                 </div>
              </div>
           </div>
        )}
           ...
           <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {store.tables.map((t: Table) => (
              <div key={t.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between min-h-[140px]">
                ...
                <button onClick={() => setResetTableId(t.id)} className="w-full py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase italic">Reset</button>
              </div>
            ))}
          </div>
        )}
