...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    // Chỉ xử lý thông báo nếu user đã đăng nhập và bật loa
    if (!currentUser || !isAudioEnabled || currentUser.role === UserRole.CUSTOMER) return;

    // Lọc thông báo CHƯA ĐỌC và PHÙ HỢP với vai trò người dùng hiện tại
    // Admin mặc định nhận mọi thông báo hệ thống trừ thông báo bếp (để tránh loãng)
    const relevantNotifs = store.notifications.filter(n => {
      if (n.read) return false;
      if (n.targetRole === currentUser.role) return true;
      if (currentUser.role === UserRole.ADMIN) {
        // Admin nhận các yêu cầu cần duyệt
        return n.type === 'qr_request' || n.type === 'move_request' || n.type === 'payment';
      }
      return false;
    });
    ...
    if (relevantNotifs.length > 0) {
      ...
      // Tránh lặp lại thông báo cũ
      if (latest.id !== lastNotifIdRef.current) {
        lastNotifIdRef.current = latest.id;
        
        // 1. Rung thiết bị (Dành cho điện thoại)
        if ('vibrate' in navigator) {
          navigator.vibrate([300, 100, 300]);
        }

        // 2. Phát giọng nói thông báo
          ...
          <div className="flex items-center gap-4">
            {currentUser && currentUser.role !== UserRole.CUSTOMER && (
              <button 
                ...
                title={isAudioEnabled ? "Tắt loa & rung" : "Bật loa & rung"}
              >
