...
import { Package, Utensils, MessageSquare, Pizza, ChevronRight, XCircle, CheckCircle, AlertTriangle } from 'lucide-react';
      ...
      {tab === 'ORDERS' ? (
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-10">
          {kitchenCommands.length > 0 && (
            <div className="bg-red-600 text-white p-6 rounded-[2.5rem] shadow-2xl relative overflow-hidden animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div key={item.id} className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden group hover:border-slate-800 transition-all">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <h3 className="text-lg font-black text-slate-800 uppercase">{item.name}</h3>
                                </div>
                                <span className="text-3xl font-black text-slate-900 bg-slate-50 px-3 py-1 rounded-xl">x{item.quantity}</span>
                            </div>
                            <button onClick={() => store.updateOrderItemStatus(item.tableId, item.id, OrderItemStatus.COOKING)} className="w-full bg-slate-900 text-white py-4 rounded-xl font-black text-xs uppercase shadow-xl active:scale-95 transition-all">Bắt đầu làm</button>
                        </div>
                    ))
                }
            </section>
            ...
            <section className="space-y-4">
                <h2 className="text-xl font-black flex items-center gap-3 text-orange-600 italic uppercase tracking-tighter"><span className="bg-orange-500 text-white w-10 h-10 rounded-xl flex items-center justify-center shadow-lg">2</span> Đang thực hiện</h2>
        ...
        <div className="flex-1 bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm flex flex-col overflow-hidden">
            <div className="flex items-center gap-4 mb-8 bg-slate-50 p-4 rounded-2xl shrink-0">
               ...
               <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Tìm kiếm món để cập nhật tình trạng..." className="bg-transparent border-none outline-none font-bold text-sm uppercase" />
            </div>
