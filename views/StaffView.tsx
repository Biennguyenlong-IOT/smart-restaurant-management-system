...
const StaffView: React.FC<StaffViewProps> = ({ store }) => {
  ...
  const handleRequestTableQr = async (tid: number) => {
    try {
        ...
        if (e.message === "LIMIT_REACHED") {
            alert("Bạn chỉ được mở tối đa 3 bàn cùng lúc! Hãy hoàn tất thanh toán bàn cũ trước khi mở bàn mới.");
        } else {
            alert("Không thể mở bàn lúc này.");
        }
    }
  };

  const handlePlaceStaffOrder = async () => {
    if (selectedTable === null || Object.keys(cart).length === 0) return alert("Vui lòng chọn đối tượng và món ăn!");
    ...
    try {
      ...
      alert("Đã gửi đơn hàng!"); setCart({}); setSelectedTable(null); setActiveTab('TABLES');
  ...
  const handleMoveRequest = async () => {
    if (moveFromId !== null && moveToId !== null) {
        ...
        alert(`Đã gửi yêu cầu chuyển Bàn ${moveFromId} sang Bàn ${moveToId} tới Admin.`);
  ...
  const billTotal = useMemo(() => 
    store.tables.find((t:any) => t.id === showBillTableId)?.currentOrders.reduce((s:number,o:any)=>s+(o.price*o.quantity), 0) || 0
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 h-full flex flex-col">
      ...
      <div className="flex justify-between items-center bg-white px-4 py-2 rounded-2xl border border-slate-100 shadow-sm">
         <p className="text-[10px] font-black uppercase text-slate-400">Đang phục vụ: <span className={activeTableCount >= 3 ? 'text-red-500' : 'text-slate-900'}>{activeTableCount}/3 bàn</span></p>
         <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${activeTableCount >= 3 ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}></div>
            <span className="text-[9px] font-black uppercase text-slate-400">{activeTableCount >= 3 ? 'Đã đạt giới hạn' : 'Sẵn sàng nhận thêm'}</span>
         </div>
      </div>

      <div className="flex gap-2 p-1.5 bg-white rounded-2xl border border-slate-200 w-full overflow-x-auto no-scrollbar shrink-0">
        <button onClick={() => setActiveTab('TABLES')} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'TABLES' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400'}`}><Utensils size={14}/> Bàn & Khách</button>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
          ...
          <div className="space-y-8">
            <section className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                  <h2 className="text-lg font-black flex items-center gap-3 italic">🪑 Quản lý Bàn</h2>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {store.tables.map((t: Table) => {
                        ...
                        return (
                            <div key={t.id} className={`p-5 rounded-[2rem] border-2 flex flex-col items-center justify-center gap-3 relative min-h-[150px] transition-all ${
                                ...
                                isAvailable ? 'border-dashed border-slate-200 opacity-60' : 'border-slate-100 bg-slate-50 opacity-20 grayscale'
                            }`}>
                                ...
                                {isAvailable && !isRequested && t.id !== 0 && (
                                    moveFromId !== null ? (
                                        <button onClick={() => setMoveToId(t.id)} className="bg-blue-600 text-white p-2.5 rounded-xl shadow-lg animate-pulse"><ArrowRightLeft size={16}/></button>
                                    ) : (
                                        <button onClick={() => handleRequestTableQr(t.id)} className="text-[9px] font-black bg-slate-900 text-white px-3 py-2.5 rounded-xl uppercase shadow-md">Mở bàn</button>
                                    )
                                )}
                                ...
                                {isMine && !isRequested && !isAvailable && (
                                  <div className="flex flex-wrap justify-center gap-1.5">
                                    {t.sessionToken && <button onClick={() => setShowQrModalId(t.id)} className="p-2 bg-slate-900 text-white rounded-xl"><QrCode size={14} /></button>}
                                    {hasOrders && (
                                        <button onClick={() => setShowBillTableId(t.id)} className={`p-2 rounded-xl text-white ${t.status === TableStatus.PAYING ? 'bg-amber-500' : 'bg-blue-500'}`}><FileText size={14} /></button>
                                    )}
                                    {isOccupied && t.id !== 0 && (
                                        <button onClick={() => setMoveFromId(t.id)} className="p-2 bg-indigo-500 text-white rounded-xl shadow-md"><ArrowRightLeft size={14} /></button>
                                    )}
                ...
                {moveFromId !== null && (
                    <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-3xl flex items-center justify-between">
                        <p className="text-xs font-black text-blue-800 uppercase italic">
                            {moveToId === null ? `Đang chọn Bàn ${moveFromId} để chuyển. Hãy chọn bàn trống...` : `Chuyển Bàn ${moveFromId} sang Bàn ${moveToId}?`}
                        </p>
                        <div className="flex gap-3">
                            ...
                            {moveToId !== null && (
                                <button onClick={handleMoveRequest} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-lg">Gửi yêu cầu</button>
                            )}
                        </div>
                    </div>
                )}
            ...
            <section className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm">
               <h2 className="text-lg font-black mb-6 italic">🔔 Chờ bưng</h2>
               <div className="space-y-4">
                  {readyItems.length === 0 && <p className="text-center py-10 text-slate-300 font-black uppercase text-[10px] italic">Trống</p>}
                  {readyItems.map((item: any) => (
                    <div key={item.id} className="flex items-center justify-between p-5 bg-green-50 border border-green-200 rounded-[2rem]">
                        ...
                        <button onClick={() => store.updateOrderItemStatus(item.tableId, item.id, OrderItemStatus.SERVED)} className="bg-green-600 text-white px-5 py-2.5 rounded-xl text-[9px] font-black uppercase shadow-lg">Xong</button>
                    </div>
                  ))}
               </div>
            </section>
        ...
        {/* Tab ORDER và Tab PAYMENTS giữ nguyên logic đã tối ưu trước đó */}
        {activeTab === 'ORDER' && (
           <div className="flex flex-col h-full bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm">
              <div className="p-6 border-b border-slate-100 space-y-4">
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-2">
                      <button onClick={() => setSelectedTable(0)} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap border-2 ${selectedTable === 0 ? 'bg-orange-500 text-white' : 'bg-slate-50'}`}>Khách lẻ</button>
                      {store.tables.filter((t:any) => t.id !== 0).map((t:Table) => (
                        <button key={t.id} onClick={() => setSelectedTable(t.id)} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap border-2 ${selectedTable === t.id ? 'bg-slate-900 text-white' : 'bg-slate-50'}`}>Bàn {t.id}</button>
                      ))}
                  </div>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
                  {store.menu.filter((m:MenuItem) => m.name.toLowerCase().includes(searchTerm.toLowerCase())).map((m:MenuItem) => {
                    ...
                    return (
                        <div key={m.id} className={`p-3 bg-slate-50 rounded-2xl border border-white ${!m.isAvailable ? 'opacity-40' : ''}`}>
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    <img src={m.image} className="w-12 h-12 rounded-xl object-cover" />
                                    <div>
                                        <h4 className="text-[11px] font-black text-slate-800 uppercase truncate">{m.name}</h4>
              ...
              {Object.keys(cart).length > 0 && (
                <div className="p-6 bg-slate-900 text-white rounded-t-[2.5rem] shadow-2xl">
                   <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-[11px] font-black uppercase italic text-orange-500">{orderType === OrderType.TAKEAWAY ? 'Mang về' : 'Tại chỗ'}</span>
                   </div>
      ...
      {/* Modals: Bill, QR... */}
      {showBillTableId !== null && (
        <div className="fixed inset-0 z-[200] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white rounded-[3rem] p-8 max-w-sm w-full shadow-2xl animate-scaleIn border border-slate-100 max-h-[90dvh] overflow-y-auto no-scrollbar text-center">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-black italic uppercase">Bàn {showBillTableId === 0 ? 'Khách lẻ' : showBillTableId}</h3>
                ...
                <button onClick={() => { store.requestPayment(showBillTableId); setShowBillTableId(null); }} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">Yêu cầu thanh toán</button>
            </div>
        </div>
      )}
    </div>
